/** Clickable DSH-X control surface for existing terminal, schedule, and Loader services. */
...
<style>body{font:15px system-ui,sans-serif;max-width:1100px;margin:auto;padding:20px;background:#f6f7fb;color:#172033}nav{display:flex;gap:8px;margin:16px 0}button,select,input,textarea{font:inherit;padding:7px;border:1px solid #b9c1d0;border-radius:6px}button{cursor:pointer;background:#fff}button:hover{background:#eaf0ff}.tab{background:#172033;color:#fff}.panel{display:none;background:#fff;border:1px solid #d3d9e5;border-radius:10px;padding:16px}.panel.active{display:block}.row{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin:10px 0}.grow{flex:1;min-width:220px}pre{background:#111827;color:#d1fae5;padding:14px;min-height:180px;white-space:pre-wrap;border-radius:8px;overflow:auto}table{width:100%;border-collapse:collapse}td,th{padding:8px;text-align:left;border-bottom:1px solid #e1e5ed}code{font-size:12px}#message{min-height:20px;color:#526070}.danger{color:#b42318}</style>
<h1>DSH-X controls</h1><p>Live controls over the services already mounted in this process.</p><nav><button class="tab" data-tab="terminal">Terminal</button><button data-tab="schedule">Schedule</button><button data-tab="plugins">Plugins</button></nav><p id="message"></p>
...
let state={agents:[],plugins:[]}, selectedTerminal;
...
async function loadState(){state=await json(await fetch('/dsh-x/api/state'));renderAgents();renderTerminals();renderPlugins()}
