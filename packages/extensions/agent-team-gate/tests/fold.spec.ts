...
function event<T extends keyof TeamSessionEventMap>(
  type: T,
  data: TeamSessionEventMap[T],
  seq: number,
): SessionEvent & { type: T; data: TeamSessionEventMap[T] } {
  return { type, data, seq, time: seq } as unknown as SessionEvent & { type: T; data: TeamSessionEventMap[T] }
}
...
describe('Agent Teams fold', () => {
  it('folds current-team records and ignores inherited records', () => {
    ...
    const turnEvent = { type: 'turn/start', data: { turn: 1 }, seq: 5, time: 5 } as unknown as SessionEvent
    expect(isTeamEvent(turnEvent)).toBe(false)
  })
