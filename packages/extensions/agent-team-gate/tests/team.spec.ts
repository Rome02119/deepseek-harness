...
function team(ctx: Context): TeamService {
  return ctx.get('agentTeams') as unknown as TeamService
}

function teamInternals(ctx: Context): TeamServiceInternals {
  return ctx.get('agentTeams') as unknown as TeamServiceInternals
}
...
function spawn(
  ...
  return team(ctx).spawnTeammate(lead, {
    name,
...
describe('Team identity and provisioning', () => {
  ...
  it('creates fresh and fork teammates with immutable names and bounded roster size', async () => {
    ...
    expect(team(ctx).listMembers(lead).map(row => [row.name, row.context, row.status])).toEqual([
      ['lead', undefined, 'idle'],
  ...
  it('records failed provisioning durably, reserves its name, and counts it against the limit', async () => {
    ...
    expect(team(ctx).listMembers(lead)[1]).toMatchObject({
      name: 'failed-worker',
      status: 'failed',
      provider: 'missing',
    })
  ...
  it('records non-Error provider failures and contains a reversed provisioning settlement race', async () => {
    ...
    expect(team(first.ctx).listMembers(first.lead)[1]).toMatchObject({
      status: 'failed',
      diagnostics: ['string provider failure'],
    })
    await expect(team(first.ctx).sendMessage(first.lead, {
  ...
  it('validates names and permits only the Lead to create or interrupt teammates', async () => {
    ...
    expect(() => team(ctx).interrupt(worker, 'worker')).toThrow(expect.objectContaining({ code: 'TEAM_LEAD_REQUIRED' }))
    expect(team(ctx).interrupt(lead, 'worker')).toEqual({ previousStatus: 'running' })
    await waitNoAgent(ctx, worker.id)
    expect(team(ctx).interrupt(lead, 'worker')).toEqual({ previousStatus: 'inactive' })
    expect(() => team(ctx).interrupt(lead, 'lead')).toThrow(expect.objectContaining({ code: 'TEAM_INVALID_TARGET' }))
  })

  it('validates teammate text fields and pre-provisioning cancellation', async () => {
    const { ctx, lead } = await setup([])
    await expect(team(ctx).spawnTeammate(lead, {
  ...
  it('treats an ordinary fork as a new Root Team and filters inherited Team state', async () => {
    const { ctx, lead } = await setup([])
    await team(ctx).createTask(lead, { subject: 'parent task', description: 'belongs to parent' })
    ...
    expect(team(ctx).membership(handle.agent)).toMatchObject({
  ...
  it('rejects stale Agent identities and non-Team subagent children', async () => {
    ...
    if (live !== undefined) expect(team(ctx).tryMembership(live)).toBeUndefined()
    await waitNoAgent(ctx, started.childId)
    expect(() => team(ctx).membership(lead)).not.toThrow()

    const impostor = { ...lead } as Agent
    expect(team(ctx).tryMembership(impostor)).toBeUndefined()
    expect(() => team(ctx).membership(impostor)).toThrow(expect.objectContaining({ code: 'TEAM_NOT_MEMBER' }))
    ...
    expect(team(ctx).membership(orphanRoot.agent)).toMatchObject({ role: 'lead', name: 'lead' })
  ...
  it('does not reinterpret an orphaned provider child or malformed parent stream as a Team root', async () => {
    ...
    expect(team(first.ctx).tryMembership(orphan.agent)).toBeUndefined()
    ...
    expect(team(second.ctx).tryMembership(child.agent)).toBeUndefined()
    journal.state = state
    await child.dispose()
  })
})
...
describe('Team shared task DAG', () => {
  it('fails loudly when the durable numeric task id space is exhausted', async () => {
    ...
    ;(lead.session.append as unknown as (type: string, data: unknown) => void)('team/task', {
      version: 1,
      teamId: TeamId(lead.id),
      task: {
        id,
    ...
    await expect(team(ctx).createTask(lead, {
  ...
  it('bounds non-deleted tasks while retaining deleted task ids as tombstones', async () => {
    ...
    const first = await team(ctx).createTask(lead, { subject: 'first', description: 'first task' })
    await expect(team(ctx).createTask(lead, { subject: 'overflow', description: 'overflow task' }))
      .rejects.toMatchObject({ code: 'TEAM_TASK_LIMIT' })

    const deleted = await team(ctx).updateTask(lead, {
      taskId: first.id,
      expectedRevision: first.revision,
      action: 'delete',
    })
    const second = await team(ctx).createTask(lead, { subject: 'second', description: 'second task' })
    expect(deleted.status).toBe('deleted')
    expect(second.id).toBe(TeamTaskId('task-2'))
    expect(team(ctx).getTask(lead, first.id).status).toBe('deleted')
    expect(team(ctx).listTasks(lead).map(task => task.id)).toEqual([second.id])
  })
  ...
  it('enforces CAS, ownership, dependencies, transitions, and write-scope warnings', async () => {
    ...
    const first = await team(ctx).createTask(alpha, {
      subject: 'first',
      description: 'first task',
      writeScopes: ['src', './src/', 'src'],
    })
    const second = await team(ctx).createTask(beta, {
      subject: 'second',
    ...
    await expect(team(ctx).updateTask(beta, {
      taskId: second.id,
      expectedRevision: second.revision,
      action: 'claim',
    })).rejects.toMatchObject({ code: 'TEAM_TASK_BLOCKED' })

    const claimed = await team(ctx).updateTask(alpha, {
      taskId: first.id,
      expectedRevision: first.revision,
      action: 'claim',
    })
    ...
    expect(team(ctx).getTask(beta, second.id)).toMatchObject({
      ready: false,
    ...
    await expect(team(ctx).updateTask(alpha, {
      taskId: first.id,
      expectedRevision: first.revision,
      action: 'complete',
    })).rejects.toMatchObject({ code: 'TEAM_TASK_STALE_REVISION' })

    const submitted = await team(ctx).updateTask(alpha, {
      taskId: first.id,
      expectedRevision: claimed.revision,
      action: 'complete',
    })
    expect(submitted).toMatchObject({ status: 'in_review', ownerName: 'alpha' })
    expect(team(ctx).getTask(beta, second.id).ready).toBe(false)
    const completed = await team(ctx).updateTask(beta, {
      taskId: first.id,
    ...
    expect(team(ctx).getTask(beta, second.id).ready).toBe(true)
    const secondClaim = await team(ctx).updateTask(beta, {
      taskId: second.id,
      expectedRevision: second.revision,
      action: 'claim',
    })
    const released = await team(ctx).updateTask(beta, {
      taskId: second.id,
    ...
    team(ctx).interrupt(lead, 'alpha')
    team(ctx).interrupt(lead, 'beta')
  ...
  it('leases claims, renews only for the owner, and permits reclaim only after expiry', async () => {
    ...
    const task = await team(ctx).createTask(owner, { subject: 'leased', description: 'leased work' })
    vi.useFakeTimers()
    vi.setSystemTime(10_000)

    const claimed = await team(ctx).updateTask(owner, {
    ...
    await expect(team(ctx).updateTask(lead, {
    ...
    const renewed = await team(ctx).updateTask(owner, {
      taskId: task.id, expectedRevision: claimed.revision, action: 'renew',
    })
    ...
    expect(team(ctx).getTask(lead, task.id).leaseExpired).toBe(true)
    const reclaimed = await team(ctx).updateTask(lead, {
      taskId: task.id, expectedRevision: renewed.revision, action: 'claim',
    })
    ...
    vi.useRealTimers()
    team(ctx).interrupt(lead, 'lease-owner')
  ...
  it('refuses verification by a prior owner after reclaim but accepts a non-author', async () => {
    ...
    const task = await team(ctx).createTask(author, { subject: 'reclaim', description: 'reclaim attack' })
    vi.useFakeTimers()
    vi.setSystemTime(10_000)
    const claimed = await team(ctx).updateTask(author, {
      taskId: task.id, expectedRevision: task.revision, action: 'claim',
    })
    vi.setSystemTime(11_001)
    const reclaimed = await team(ctx).updateTask(reclaimer, {
      taskId: task.id, expectedRevision: claimed.revision, action: 'claim',
    })
    const submitted = await team(ctx).updateTask(reclaimer, {
      taskId: task.id, expectedRevision: reclaimed.revision, action: 'complete',
    })

    await expect(team(ctx).updateTask(author, {
      taskId: task.id,
      expectedRevision: submitted.revision,
      action: 'verify',
      receipt: receipt(author, 'reclaim-author'),
    })).rejects.toMatchObject({ code: 'TEAM_SELF_GRADING' })
    await expect(team(ctx).updateTask(verifier, {
      taskId: task.id,
      expectedRevision: submitted.revision,
      action: 'verify',
    ...
    vi.useRealTimers()
    for (const name of ['reclaim-author', 'reclaimer', 'reclaim-verifier']) team(ctx).interrupt(lead, name)
  ...
  it('refuses verification by a prior owner after Lead reassignment', async () => {
    ...
    const task = await team(ctx).createTask(author, { subject: 'reassign', description: 'reassign attack' })
    const claimed = await team(ctx).updateTask(author, {
      taskId: task.id, expectedRevision: task.revision, action: 'claim',
    })
    const reassigned = await team(ctx).updateTask(lead, {
      taskId: task.id, expectedRevision: claimed.revision, action: 'reassign', owner: 'assignee',
    })
    const submitted = await team(ctx).updateTask(assignee, {
    ...
    team(ctx).interrupt(lead, 'reassign-author')
    team(ctx).interrupt(lead, 'assignee')
  ...
  it('permits verification by an assignee who never acts on the task', async () => {
    ...
    const task = await team(ctx).createTask(lead, { subject: 'assignment', description: 'administrative assignment' })
    const assigned = await team(ctx).updateTask(lead, {
    ...
    const submitted = await team(ctx).updateTask(worker, {
      taskId: task.id, expectedRevision: reassigned.revision, action: 'complete',
    })

    await expect(team(ctx).updateTask(assignee, {
      taskId: task.id,
      expectedRevision: submitted.revision,
      action: 'verify',
    ...
    team(ctx).interrupt(lead, 'idle-assignee')
    team(ctx).interrupt(lead, 'assigned-worker')
  ...
  it('refuses verification after an assignee renews, edits, and submits the task', async () => {
    ...
    const task = await team(ctx).createTask(lead, { subject: 'assignment', description: 'contributed assignment' })
    let current = await team(ctx).updateTask(lead, {
      taskId: task.id, expectedRevision: task.revision, action: 'reassign', owner: 'acting-assignee',
    })
    current = await team(ctx).updateTask(assignee, {
    ...
    team(ctx).interrupt(lead, 'acting-assignee')
  ...
  it('retains authorship after a claimant releases the task', async () => {
    ...
    const task = await team(ctx).createTask(author, { subject: 'release', description: 'released contribution' })
    const claimed = await team(ctx).updateTask(author, {
      taskId: task.id, expectedRevision: task.revision, action: 'claim',
    })
    await team(ctx).updateTask(author, {
    ...
    team(ctx).interrupt(lead, 'releasing-author')
  ...
  it('retains unique ordered task authors through owner removal and unblock', async () => {
    ...
    const task = await team(ctx).createTask(first, { subject: 'authors', description: 'author history' })
    const authorIds = (): SessionId[] => durable(lead).tasks.find(candidate => candidate.id === task.id)!.authorIds
    vi.useFakeTimers()
    vi.setSystemTime(10_000)
    let current = await team(ctx).updateTask(first, {
      taskId: task.id, expectedRevision: task.revision, action: 'claim',
    })
    vi.setSystemTime(11_001)
    current = await team(ctx).updateTask(second, {
      taskId: task.id, expectedRevision: current.revision, action: 'claim',
    })
    current = await team(ctx).updateTask(lead, {
    ...
    current = await team(ctx).updateTask(third, {
    ...
    vi.useRealTimers()
    for (const name of ['first-author', 'second-author', 'third-author']) team(ctx).interrupt(lead, name)
  ...
  it('blocks five failed verifications until the Lead resets the task', async () => {
    ...
    let current = await team(ctx).createTask(lead, { subject: 'cap', description: 'attempt cap' })
    const dependent = await team(ctx).createTask(lead, {
      subject: 'dependent', description: 'waits for cap task', blockedBy: [current.id],
    })

    for (let attempt = 1; attempt <= 5; attempt += 1) {
      const claimed = await team(ctx).updateTask(lead, {
        taskId: current.id, expectedRevision: current.revision, action: 'claim',
      })
      const reviewed = await team(ctx).updateTask(lead, {
        taskId: current.id, expectedRevision: claimed.revision, action: 'complete',
      })
      current = await team(ctx).updateTask(verifier, {
        taskId: current.id,
    ...
    expect(team(ctx).getTask(lead, dependent.id).ready).toBe(false)
    await expect(team(ctx).updateTask(verifier, {
      taskId: current.id,
      expectedRevision: current.revision,
      action: 'verify',
  ...
  it('blocks three consecutive failure signatures and resets stagnation when the signature changes', async () => {
    ...
    let current = await team(ctx).createTask(lead, { subject: 'sig', description: 'signature cap' })
    ...
    team(ctx).interrupt(lead, 'sig-verifier')
    await waitNoAgent(ctx, verifier.id)
  })
  ...
  it('rejects invalid verification and records a failed clean gate for retry', async () => {
    ...
    const task = await team(ctx).createTask(author, { subject: 'gate', description: 'gate' })
    ...
    const submitted = await team(ctx).updateTask(author, {
    ...
    await expect(team(ctx).updateTask(peer, {
      taskId: task.id,
      expectedRevision: submitted.revision,
      action: 'verify',
    ...
    const failed = await team(ctx).updateTask(verifier, {
      taskId: task.id,
      expectedRevision: submitted.revision,
      action: 'verify',
    ...
    team(ctx).interrupt(lead, 'author')
    team(ctx).interrupt(lead, 'peer')
    team(ctx).interrupt(lead, 'verifier')
  ...
  it('derives verifier identity so accepted receipts remain replayable', async () => {
    ...
    const completed = await team(ctx).updateTask(verifier, {
      taskId: task.id,
      expectedRevision: submitted.revision,
      action: 'verify',
  ...
  it('enforces requiresProvider on task claim, reassign, and unrestricted claim when undeclared', async () => {
    ...
    const forkTask = await team(ctx).createTask(lead, {
      subject: 'fork only',
    ...
    await expect(team(ctx).updateTask(spawnWorker, {
      taskId: forkTask.id,
      expectedRevision: forkTask.revision,
      action: 'claim',
    })).rejects.toMatchObject({ code: 'TEAM_PROVIDER_MISMATCH' })

    const claimedByFork = await team(ctx).updateTask(forkWorker, {
      taskId: forkTask.id,
      expectedRevision: forkTask.revision,
      action: 'claim',
    })
    ...
    const releasedFork = await team(ctx).updateTask(forkWorker, {
      taskId: forkTask.id,
    ...
    const reassignedFork = await team(ctx).updateTask(lead, {
      taskId: forkTask.id,
      expectedRevision: releasedFork.revision,
      action: 'reassign',
      owner: 'fork-worker',
    ...
    const freeTask = await team(ctx).createTask(lead, {
      subject: 'unrestricted',
      description: 'no provider requirement',
    })
    expect(freeTask.requiresProvider).toBeUndefined()

    const freeClaimedSpawn = await team(ctx).updateTask(spawnWorker, {
      taskId: freeTask.id,
    ...
    const freeReleased = await team(ctx).updateTask(spawnWorker, {
      taskId: freeTask.id,
      expectedRevision: freeClaimedSpawn.revision,
      action: 'release',
    })

    const freeClaimedFork = await team(ctx).updateTask(forkWorker, {
      taskId: freeTask.id,
    ...
    const editedFork = await team(ctx).updateTask(forkWorker, {
      taskId: freeTask.id,
    ...
    team(ctx).interrupt(lead, 'spawn-worker')
    team(ctx).interrupt(lead, 'fork-worker')
  ...
  it('rejects a provisioning teammate before verification constructs a receipt', async () => {
    const { ctx, lead } = await setup([])
    const task = await team(ctx).createTask(lead, { subject: 'gate', description: 'gate' })
    const claimed = await team(ctx).updateTask(lead, {
      taskId: task.id, expectedRevision: task.revision, action: 'claim',
    })
    const submitted = await team(ctx).updateTask(lead, {
      taskId: task.id, expectedRevision: claimed.revision, action: 'complete',
    })
    ...
    await expect(team(ctx).updateTask(verifier.agent, {
      taskId: task.id,
      expectedRevision: submitted.revision,
      action: 'verify',
  ...
  it('rejects malformed scopes and every invalid dependency relation', async () => {
    const { ctx, lead } = await setup([])
    const first = await team(ctx).createTask(lead, { subject: 'one', description: 'one' })
    const second = await team(ctx).createTask(lead, {
  ...
  it('rejects incomplete mutations, invalid transitions, and deletion of a live blocker', async () => {
    const { ctx, lead } = await setup([])
    await expect(team(ctx).createTask(lead, { subject: ' ', description: 'invalid' }))
      .rejects.toMatchObject({ code: 'TEAM_INVALID_ARGUMENT' })
    await expect(team(ctx).createTask(lead, { subject: 'invalid', description: '' }))
      .rejects.toMatchObject({ code: 'TEAM_INVALID_ARGUMENT' })
    await expect(team(ctx).createTask(lead, { subject: 'x'.repeat(201), description: 'too long' }))
      .rejects.toMatchObject({ code: 'TEAM_INVALID_ARGUMENT' })
    const blocker = await team(ctx).createTask(lead, { subject: 'blocker', description: 'blocker' })
    await team(ctx).createTask(lead, {
      subject: 'dependent', description: 'dependent', blockedBy: [blocker.id],
    })
    expect(() => team(ctx).getTask(lead, TeamTaskId('missing')))
  ...
  it('supports Lead reassignment, completion, reopen, and deletion permissions', async () => {
    ...
    const task = await team(ctx).createTask(owner, { subject: 'lifecycle', description: 'lifecycle' })
    const assigned = await team(ctx).updateTask(lead, {
      taskId: task.id,
      expectedRevision: task.revision,
      action: 'reassign',
    ...
    await expect(team(ctx).updateTask(owner, {
      taskId: task.id,
      expectedRevision: assigned.revision,
      action: 'reassign',
      owner: 'lead',
    })).rejects.toMatchObject({ code: 'TEAM_LEAD_REQUIRED' })
    const complete = await team(ctx).updateTask(owner, {
      taskId: task.id,
      expectedRevision: assigned.revision,
      action: 'complete',
    })
    ...
    const reopened = await team(ctx).updateTask(owner, {
      taskId: task.id,
      expectedRevision: complete.revision,
      action: 'reopen',
    })
    ...
    const deleted = await team(ctx).updateTask(owner, {
      taskId: task.id,
      expectedRevision: claimed.revision,
      action: 'delete',
    })
    expect(deleted.status).toBe('deleted')
    expect(team(ctx).listTasks(lead)).toEqual([])
    await expect(team(ctx).updateTask(owner, {
      taskId: task.id,
  ...
  it('covers partial edits, Lead ownership, unassignment, and blocked reassignment', async () => {
    ...
    const blocker = await team(ctx).createTask(lead, { subject: 'blocker', description: 'blocker' })
    const task = await team(ctx).createTask(lead, {
      subject: 'draft',
    ...
    const subject = await team(ctx).updateTask(editor, {
      taskId: task.id, expectedRevision: assigned.revision, action: 'edit', subject: 'edited subject',
    })
    const description = await team(ctx).updateTask(editor, {
      taskId: task.id,
      expectedRevision: subject.revision,
      action: 'edit',
      description: 'edited description',
    })
    const scopes = await team(ctx).updateTask(editor, {
      taskId: task.id,
    ...
    const unassigned = await team(ctx).updateTask(lead, {
    ...
    expect(team(ctx).getTask(lead, broad.id).writeScopeWarnings)
      .toEqual([`write scopes overlap with ${narrow.id}`])

    team(ctx).interrupt(lead, 'editor')
...
describe('Team mailbox and waiting', () => {
  it('acknowledges waking messages persisted by a busy Lead before model claim', async () => {
    ...
    const first = await team(ctx).sendMessage(reporter, {
      target: 'lead', content: content('first wakeup report'), delivery: 'wakeup', signal: SIGNAL,
    })
    const second = await team(ctx).sendMessage(reporter, {
  ...
  it('flushes a live pending receipt before acknowledgement without inserting a duplicate', async () => {
    ...
    const immediate = await team(ctx).sendMessage(lead, {
    ...
    team(ctx).interrupt(lead, 'pending-target')
  ...
  it('acknowledges waking messages accepted by a busy target inbox', async () => {
    ...
    const first = await team(ctx).sendMessage(lead, {
    ...
    flushed.length = 0
    const second = await team(ctx).sendMessage(lead, {
    ...
    team(ctx).interrupt(lead, 'busy-target')
    target.cancel({ kind: 'parent' })
    await waitNoAgent(ctx, target.id)
  })
  ...
  it('deduplicates live target history and contains inspection and delivery failures', async () => {
    ...
    const uncertain = await team(ctx).sendMessage(lead, {
    ...
    const failed = await team(ctx).sendMessage(lead, {
    ...
    team(ctx).interrupt(lead, 'live-target')
  ...
  it('keeps quiet mail dormant, wakes on follow-up, preserves FIFO, and de-duplicates delivery', async () => {
    ...
    const quiet = await team(ctx).sendMessage(alpha, {
    ...
    const waking = await team(ctx).sendMessage(alpha, {
  ...
  it('interrupts only the current turn and retains an already accepted follow-up', async () => {
    ...
    const started = await spawn(ctx, lead, 'worker')
    const worker = await waitRunning(ctx, started.member.id)
    const followup = await team(ctx).sendMessage(lead, {
  ...
  it('disposes live teammate Activations and their waits when the Team service unloads', async () => {
    ...
    const waiting = team(ctx).waitForChange(lead, 10_000, SIGNAL)
  ...
  it('disposes a live child even after its durable member edge becomes failed', async () => {
    ...
    await ctx.sessions.flush(lead.session)
    expect(team(ctx).listMembers(lead)[1]?.status).toBe('failed')

    const internal = team(ctx) as unknown as { disposeRuntime(): Promise<void> }
  ...
  it('aborts and awaits an admitted cold mailbox dispatch during disposal', async () => {
    ...
    const sending = team(ctx).sendMessage(lead, {
...
describe('Enforced write scopes', () => {
  it('normalizes traversal before enforcing writeScopes', async () => {
    ...
    const task = await team(ctx).createTask(worker, {
    ...
    await team(ctx).updateTask(worker, {
      taskId: task.id, expectedRevision: task.revision, action: 'claim',
    })
    ...
    team(ctx).interrupt(lead, 'worker')
    await waitNoAgent(ctx, worker.id)
  })
  ...
  it('permits writes when no owned active task declares writeScopes', async () => {
    ...
    const claimed = await team(ctx).updateTask(worker, {
      taskId: task.id, expectedRevision: task.revision, action: 'claim',
    })
    ...
    team(ctx).interrupt(lead, 'unrestricted-worker')
    await waitNoAgent(ctx, worker.id)
  })
