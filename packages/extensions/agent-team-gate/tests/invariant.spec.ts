...
function appendTeam(session: Session, type: string, data: unknown): void {
  (session.append as unknown as (t: string, d: unknown) => void)(type, data)
}
...
describe('Agent Teams stream invariant', () => {
  it('accepts provisioning and rejects a terminal member as the first edge', async () => {
    ...
    expect(() => {
      appendTeam(session, 'team/member', { version: 1, teamId: TeamId(session.id), member })
    }).not.toThrow()

    const invalid = ctx.sessions.create(SessionId('team-invariant-invalid'))
    expect(() => {
      appendTeam(invalid, 'team/member', {
        version: 1,
      ...
      code: 'INVARIANT',
      packageName: '@deepseek-ai/dsh-agent-team-gate',
    }))
  ...
  it('accepts an in-review task with a verification receipt', async () => {
    ...
    expect(() => {
      ...
      appendTeam(session, 'team/task', { version: 1, teamId, task: { ...task, revision: 1, status: 'pending' } })
      appendTeam(session, 'team/task', { version: 1, teamId, task: { ...task, revision: 2, status: 'in_progress' } })
      appendTeam(session, 'team/task', {
        version: 1,
        teamId,
