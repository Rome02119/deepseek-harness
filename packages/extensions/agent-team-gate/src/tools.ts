...
/** Cordis plugin name. */
export const name = 'tool-agent-team-gate'
...
const TASK_RECEIPT_VIEW_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  properties: {
 ...
 * @param agent - Calling Agent instance whose scope receives the tool set.
 * @param ctx - Cordis Context hosting the AgentTeams service.
 * @param config - Provider configuration for fresh and fork teammates.
 * @returns Disposer function that unregisters all installed tools and prompts.
 */
export function installTeamTools(agent: Agent, ctx: Context, config: Config = {}): () => void {
  ...
  try {
    ...
    register(scoped.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return await ctx.agentTeams.spawnTeammate(agent, {
          ...
          context,
          provider: context === 'fork' ? resolved.forkProvider : resolved.freshProvider,
          signal: exec.signal,
        })
      },
    })))
    ...
    register(scoped.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        return await ctx.agentTeams.updateTask(caller, {
          ...
          ...args.receipt === undefined ? {} : {
            receipt: {
              ...
              verifierName: ctx.agentTeams.membership(caller).name,
              command: args.receipt.command,
              exitCode: args.receipt.exit_code,
              gitSha: args.receipt.git_sha,
              branch: args.receipt.branch,
              dirty: args.receipt.dirty,
              outputDigest: args.receipt.output_digest,
              ...args.receipt.worker_provider === undefined ? {} : { workerProvider: args.receipt.worker_provider },
              ...args.receipt.verifier_provider === undefined ? {} : { verifierProvider: args.receipt.verifier_provider },
            },
          },
        })
      },
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  const maybeInstall = (agent: Agent): void => {
    ...
    installed.set(agent, installTeamTools(agent, ctx, config))
  }
  ...
  ctx.effect(() => () => {
    ...
    installed.clear()
  }, 'tool-agent-team-gate.scopedTools()')
}
