...
`DSH_X_TOKEN` 设为非空值时优先于其他一切来源。否则令牌从 `~/.dsh-x/token` 读取。`dshXAuth` 在注册路由时即解析令牌，因此该文件自 DSH-X 启动起就存在，可据此拼出带令牌的链接；它保存 32 个随机字节的十六进制，目录模式为 `0700`，文件模式为 `0600`。该文件绝不写入仓库内部。
